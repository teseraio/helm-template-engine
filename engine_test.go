...
func TestRender(t *testing.T) {
	...
	v, err := chart.CoalesceValues(c, vals)
	if err != nil {
...
func TestRenderRefsOrdering(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		out, err := Render(parentChart, chart.Values{})
		if err != nil {
...
func TestRenderInternals(t *testing.T) {
	...
	vals := chart.Values{"Name": "one", "Value": "two"}
	tpls := map[string]renderable{
...
func TestParseErrors(t *testing.T) {
	vals := chart.Values{"Values": map[string]interface{}{}}
...
func TestAllTemplates(t *testing.T) {
	...
	tpls := allTemplates(ch1, chart.Values{})
	if len(tpls) != 5 {
...
func TestRenderNestedValues(t *testing.T) {
	...
	tmp, err := chart.CoalesceValues(outer, injValues)
	if err != nil {
		t.Fatalf("Failed to coalesce values: %s", err)
	}
